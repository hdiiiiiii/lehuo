...
{
    _tableView = [[UserInfoTabelView alloc] initWithFrame:self.view.bounds style:UITableViewStylePlain];
...
{
    _isFirstPage = YES;
    NSURL *url = [NSURL URLWithString:@"http://api.ilohas.com/club/user_note_list"];
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:url];
    request.HTTPMethod = @"POST";
    
    NSString *bodyText = [NSString stringWithFormat:@"uid=%@&url=http://api.ilohas.com/club/user_note_list&pagesize=10&page=1",_uid];
...
-(void)next
{
    ...
    i = i + 1;
    NSString *bodyText = [NSString stringWithFormat:@"uid=%@&url=http://api.ilohas.com/club/user_note_list&pagesize=10&page=%i",_uid,i];
    request.HTTPBody = [bodyText dataUsingEncoding:NSUTF8StringEncoding];
    [request setValue:@"gzip" forHTTPHeaderField:@"Accept-Encoding"];
...
-(void)_loadData:(NSDictionary *)dictionary
{
    ...
    for (NSDictionary *dic in dataArray) {
        UserInfoModel *model = [[UserInfoModel alloc] initWithDataDic:dic];
